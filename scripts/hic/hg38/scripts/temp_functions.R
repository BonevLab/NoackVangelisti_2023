...
plotMeth <- function(methData, plotOrder, plotMar=c(3.5,3),vertical=FALSE, plotAnn=F, plotStart=F, yLim=c(0,100),cex.axis,setNames=NA,methColors=NA,figure_mode=FALSE,methIndex=1,currentIntrv){
  ...
  if(vertical == TRUE)
  {
    ...
    for (meth in cNames)
    {
      ...
      currentLim <- yLim
      plot(x = currentData$start+1,y=currentData[,4],xlim=c(currentIntrv$start,currentIntrv$end),ylim=c(-5,100),pch=20,cex=1.2, cex.main=cex.axis, xlab='', ylab='', yaxt='n', xaxt='n', col=col,main=ifelse(figure_mode,'',setName),xaxs='i',bty="n")
      mtext(setName, side = 4, outer = F,line=0.5,adj=0,las=2,cex=cex.axis*1.2)
