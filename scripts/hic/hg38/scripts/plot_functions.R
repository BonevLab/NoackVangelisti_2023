...
#' @export
##########################################################################################################
plot_averageTAD <- function(tracks=all_tracks,add_matrix=NULL,mat_lim=NULL,cells,domains=NULL,domain_size=c(1e5,2e6),path,file_f,stats_f=T,plot_what='',zlim=c(-1.5,1.5),z_colors=wide_red_blue_pal(1000),flip=T,cell_colors=rainbow(length(cells)),width=1.5*length(cells),height=6,norm='norm',fig_name=NULL,add_plot=F,par_mar = c(0.5, 1, 0.5, 0)){
	...
	colnames(df_nm) <- paste0(rep(cells,2),c(rep('intra',length(cells)),rep('inter',length(cells))))
	pdf(fig_name,width=width,height=height)
	df <- melt(as.data.frame(df_nm))
	df$cell <- factor(rep(rep(cells,each=nrow(df_nm)),2),level=cells)
	df$type <- factor(rep(c('intraTAD','interTAD'),each=2*nrow(df_nm)),levels=c('intraTAD','interTAD'))
	df$variable <- factor(df$variable,levels=colnames(df_nm))
	p1 <- ggplot(df,aes(x=cell,y=value,fill=cell)) + geom_boxplot(outlier.size=1,show.legend = F,width=0.8,outlier.shape = NA) + scale_x_discrete(labels=gsub('E14_','',rep(cells,2))) + facet_wrap(~type)
	...
	p1 <- p1 + stat_compare_means(comparisons = list(c(cells[1],cells[2])),label = "p.signif",method='wilcox',label.y=c(quantile(df_nm,0.95,na.rm=T)*2,quantile(df_nm,0.95,na.rm=T)*2.25,quantile(df_nm,0.95,na.rm=T)*2.5),tip.length=0.005)
#	p1 <- p1 + stat_compare_means(comparisons = list(c(colnames(df_nm)[4],colnames(df_nm)[5]),c(colnames(df_nm)[5],colnames(df_nm)[6]),c(colnames(df_nm)[4],colnames(df_nm)[6])),label = "p.signif",method='wilcox',label.y=c(quantile(df_nm,0.7,na.rm=T)+0.2,quantile(df_nm,0.7,na.rm=T)+0.1,quantile(df_nm,0.7,na.rm=T)+0.32),tip.length=0.005)
	...
	dev.off()
		...
		if (!is.null(add_matrix)&!(add_plot)){
		  layout(mat = matrix(c(1:(length(cells)*2+2)), nrow = 2, ncol = length(cells)+1,byrow = F),heights = c(4, 1.5),widths = c(rep(8,length(cells)),1),respect=F) 
		} else if (!add_plot){
		  layout(mat = matrix(c(1:(length(cells)+1)), nrow = 1, ncol = length(cells)+1,byrow = F),heights = 4,widths = c(rep(4,length(cells)),1),respect=F) 
		}
		for (cell in cells){
		...
		if(!add_plot){
		  par(mar=c(1,1,1,3))
		  image.scale(as.matrix(1),zlim=zlim, col=z_colors,axis.pos=4,adj=0.5,cex.axis = 1.2)
		}
	}
	if(!add_plot){
	  dev.off()
		...
		for (grid_fn in grid_fns){
		  image_file = paste0(path,basename(grid_fn),'_',plot_res/1000,'kb.pdf')
			...
			for (i in which_plot) {
			   ...
			   if(!use_score){
			   if(plot_mean){
			     center_v <- sum(o[(floor(ncol(o)/2)):(floor(ncol(o)/2)+2),(floor(ncol(o)/2)):(floor(ncol(o)/2)+2)]*grid$total_obs[i],na.rm=T)/(sum(e[(floor(ncol(e)/2)):(floor(ncol(e)/2)+2),(floor(ncol(e)/2)):(floor(ncol(e)/2)+2)]*grid$total_exp[i],na.rm=T)/2)
			     left_top <- sum(o[1:3,1:3]*grid$total_obs[i],na.rm=T)/(sum(e[1:3,1:3]*grid$total_exp[i],na.rm=T)/2)
			     left_bottom <- sum(o[(nrow(o)-2):(nrow(o)),1:3]*grid$total_obs[i],na.rm=T)/(sum(e[(nrow(o)-2):(nrow(o)),1:3]*grid$total_exp[i],na.rm=T)/2)
			     right_top <- sum(o[1:3,(ncol(o)-2):(ncol(o))]*grid$total_obs[i],na.rm=T)/(sum(e[1:3,(ncol(o)-2):(ncol(o))]*grid$total_exp[i],na.rm=T)/2)
			     right_bottom <- sum(o[(nrow(o)-2):(nrow(o)),(ncol(o)-2):(ncol(o))]*grid$total_obs[i],na.rm=T)/(sum(e[(nrow(o)-2):(nrow(o)),(ncol(o)-2):(ncol(o))]*grid$total_exp[i],na.rm=T)/2)
...
plotMext <- function(
  ...
  ln.h=NULL, colvec=NULL, pointsize=12, par_mar=c(5.1,4.1,4.1,2.1),...) {
...
plotBinned_CpG_GpC <- function(trackdb=trackdb,extra_tracks2=atac_tracks,extra_tracks=eigen_tracks,extractWhat='avg',cells=cells,init_cell=cells[1],chrs=chrs,region=gintervals.2d(chrs),binSize=binSize,path=paste0(main_f,'analysis/compartments/'),file_f='test.pdf',cutoff_value=NULL,obs_colors=colorRampPalette(c("white","orange","red","darkRed")),norm_colors=wide_red_blue_pal,plot_what=c('obs','norm'),zlim_norm=c(-3,3),balance=F,fig_name=NULL,width,height){
  for (t in 1:length(extra_tracks)){
    gvtrack.create(paste0('v_',extra_tracks[t]), extra_tracks[t], extractWhat)
    
  }
  
  for (t in 1:length(extra_tracks2)){
    gvtrack.create(paste0('v2_',extra_tracks2[t]), extra_tracks2[t], extractWhat)
  ...
  pdf(fig_name,width=width*length(cells),height=height)
  layout(mat = matrix(c(1:(length(cells)*2+5)), nrow = 3, ncol = length(cells)+1,byrow = F),heights = c(4, 0.5,0.5),widths = c(rep(4,length(cells)),0.5),respect=T) 
  #layout(matrix(1:((length(tracks)+1)*length(cells)),length(tracks)+1,length(cells),byrow=F),c(rep(4,length(cells)),1), c(4,rep(0.5,length(tracks))))
  
  for (cell in cells){
    split<-as.data.frame(strsplit(cell,'_'))
    tracks <- extra_tracks[grep(split[3,],extra_tracks)]
    tracks2 <- extra_tracks2[grep(split[3,],extra_tracks2)]
    init_temp <- df_list[[init_cell]]
    temp_list <- df_list[[cell]]
    for (chr in chrs){
      if (!balance) {
      ...
      df_cast_norm [df_cast_norm>zlim_norm[2]] <- zlim_norm[2]
      message(max_value)
      df_tracks <- gextract(paste0('v_',tracks[grep(split[3,],tracks)]),intervals=gintervals(region[,1],region[,2],region[,3]),iterator=binSize)
      df_tracks2 <- gextract(paste0('v2_',tracks2[grep(split[3,],tracks2)]),intervals=gintervals(region[,1],region[,2],region[,3]),iterator=binSize)
      ...
      if (grepl('obs',plot_what)){
        ...
        #hm <- Heatmap(as.matrix(df_cast_obs),cluster_columns = F,cluster_rows=F,show_row_names = F,show_column_names = F,col=obs_colors(1000),show_heatmap_legend = F,raster_quality = 1)
        image(rotate(as.matrix(df_cast_obs)), zlim=c(0,max_value),  col=obs_colors(1000),xaxt="n",yaxt="n",xlab='', ylab='',useRaster=TRUE) #plot the image
        #text(gsub('3DRAM_D45_','',cell),x=.9,y=.9,font=2,cex = 2)
        for (i in 1:length(tracks)){
          min_label <- round(min(df_tracks[,i+3],na.rm=T),1)
          max_label <- round(max(df_tracks[,i+3],na.rm=T),1)
          par(xaxs="i")
          ...
          if (grepl('eigen',tracks[i])){
            ...
            if(which(cells%in%cell)==1){
              axis_labels <- c(60,100)
              axis_labels2 <- c(0,10)
            } else {
              axis_labels <- NA
            }
            plot(x=1:nrow(df_tracks),y=rollmean(df_tracks[,i+3],3,na.pad=T),lwd=1,type='l',ylim=c(60,100),col='red',bty ='n',xlab='', ylab='', yaxt='n', xaxt='n',cex.lab=1.3)
            axis(2, labels=axis_labels,at=c(60,100),outer = F, cex.axis=1, las=2)
            
            plot(x=1:nrow(df_tracks2),y=rollmean(df_tracks2[,i+3],3,na.pad=T),lwd=1,type='l',ylim=c(0,10),col='blue',bty ='n',xlab='', ylab='', yaxt='n', xaxt='n',cex.lab=1.3)
            axis(2, labels=axis_labels2,at=c(0,10),outer = F, cex.axis=1, las=2)
