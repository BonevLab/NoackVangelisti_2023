...
#' @export
##########################################################################################################
rank_matrix <- function(trackdb=trackdb,tracks=all_tracks,eigen_tracks=eigen_tracks,cells=cells,chrs=gintervals.all()$chrom,path=paste0(main_f,'/data/extractedBins/'),binSize=1e5,min_dist=1e7,ranks=50,file_f=file_f){
...
run_cisDecay <-function(tracks=all_tracks,cells=cells,path=paste0(main_f,'analysis/cis_decay/'),log_base=2,norm.by.binsize=T,norm.by.total.obs=T,file_f='test',minDist=1000,maxDist=2e8,coeff_range=c(1e5,2e6),colors=rainbow(length(cells)),out_f=paste0(main_f,'analysis/cis_decay/cisDecay_log',log_base,'.pdf'),cellsToPlot=cells,alpha=0.7,disp_colors=NULL,labels=NULL,width=8,height=8){
  ...
  if (log_base==2){
    ...
    for (i in 1:nrow(d_means)){
      if(!is.na(mean(as.numeric(d_sterr[i,2:10]),na.rm=T))){
    ...
    legend(x="topleft",d_means$type,cex=1,lwd=rep(1,length(cellsToPlot)),col=as.vector(d_means$colors))
    dev.off()
    ...
    a_log$type <- factor(a_log$type,levels=cellsToPlot)
    ...
    plot(x=dist,y=d_means[1,2:(ncol(d_means)-3)],ylim=c((min_mean-min_sterr),(max_mean+max_sterr)),type='n',xlab="distance(log10)",ylab="Relative contact probability",xaxt='n',xlim=c(dist[findInterval(minDist,dist_f)+1],dist[findInterval(maxDist,dist_f)-1]),cex.axis=1.5,cex.lab=1.5)
    minor.ticks.axis(1,9,mn=3,mx=8,cex.axis=1.5,cex.lab=1.5)
    for (i in 1:nrow(d_means)){
      if(!is.na(mean(as.numeric(d_sterr[i,2:10]),na.rm=T))){
    ...
    legend(x="topright",legend = as.character(labels),cex=1.5,lwd=rep(2,length(cellsToPlot)),col=as.vector(d_means$colors),seg.len = 1)
...
#' @export
##########################################################################################################
analyzeInsulation <- function(tracks,rep_tracks,cells,min_nreps=NULL,border_max_distance=1e4,path=path,ins_dom_thresh=0.1,domain_size_range=c(5e4, 4e6),sig_thresh=0.8,no_sig_thresh=0.1,use.normFACS=TRUE,use.sigThresh=TRUE,write.insTracks=F,write.borders=F,anyDiff=F){
  borders = c()
  tracks <- as.vector(unlist(sapply(cells,function(x){tracks[grep(x,tracks)]})))
  ...
  rep_stats <- list()
  number_matrix <- matrix(nrow=length(tracks),ncol=4)
  size_matrix <- matrix(nrow=length(tracks),ncol=4)
  ...
  for (cell in cells) {
    ...
    if(!is.null(min_nreps)){
      ...
      track_borders <- adply(rep_ins,1,function(x){
        if(sum(x[,4:(ncol(x)-1)]<quantiles)>=min_nreps) {return(x)}
      })
      message("Track:",ins_track,", N borders in at least ",min_nreps," replicates:",nrow(track_borders))
    }
