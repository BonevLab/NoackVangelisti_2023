...
binSize=2e5
chrs=c('chr4')
file_f <- paste0(path,'chr4_',binSize/1000,'kb')

test <- extractBinned(tracks=all_tracks,cells=cells,chrs=chrs,path=path,binSize=binSize,file_f=file_f)
plotBinned(extra_tracks=eigen_tracks,cells=cells,init_cell=cells[1],chrs='chr4',binSize=binSize,path=path,file_f=file_f,plot_what='obs',balance=T,fig_name = '/home/hpc/bonev/projects/hic/test/cooltools/flo/PancreaticCancer_3DRAM.pdf',width=5,height=5)
...
res <- analyzeCompartments(cells=cells,ins_dom_thresh=0.1,insulation=250,min_domainSize = 5e4)
...
res <- analyzeInsulation(tracks=ins_tracks,nrep=3,rep_tracks=ins_rep_tracks,path=paste0(main_f,'analysis/insulation/'),cells=cells,ins_dom_thresh=0.1,border_max_distance=1e4,sig_thresh=0.8,no_sig_thresh=0.2,use.normFACS=TRUE,use.sigThresh=TRUE,write.insTracks=F,write.borders=T,anyDiff=T)
...
for (cell in cells[4:5]){
  submit_aggregateHiC(cells=cell,tracks=all_tracks,range_f=40000,res_f=1000,filter_f=0,
                      intervals1='/home/hpc/bonev/data/hic/data/peaks/final/NPC_Sox2_peaks.narrowPeak',
                      intervals2='/home/hpc/bonev/data/hic/data/peaks/final/NPC_Sox2_peaks.narrowPeak',
                      domains=paste0("hic.ncx_Hes5.ins_250_domains_expanded"),
                      grid_mode='1D',mem=80)
  }
