...
plotSimpleBED <- function(set, plotMar=c(3.5,3), plotLim,cols=c('purple','#D4AF37')){
  par(mar = c(0.2, plotMar[1], 0.2, plotMar[2]))
  
  plot(0 ,ylim=c(0,50), xlim=plotLim, xlab='', ylab='', yaxt='n', xaxt='n',yaxs='i',xaxs='i',frame=F)
  
  for (i in 1:nrow(set))
  {
    rect(set[i,2], 0, set[i,3], 50, col='black', border = NA)
...
plotArcs <- function(arcIntervals,plotMar=c(3.5,3),color_by,cols=colorRampPalette(c('blue','white','red')),plotLim,flip=F,plotScale=T){
  par(mar = c(0, plotMar[1], 0.1, plotMar[2]))
...
plotMeth <- function(methData, plotOrder, plotMar=c(3.5,3),vertical=FALSE, plotAnn=F, plotStart=F, yLim=c(0,100),cex.axis,setNames=NA,methColors=NA,figure_mode=FALSE,methIndex=1,currentIntrv){
  ...
  if(vertical == TRUE)
  {
    ...
    for (meth in cNames)
    {
      i=1
      if(is.na(setNames)){
        setName=gsub('v_methylation.|v_methseq_RPM.|v_hic.','',meth)
      } else {
        setName=setNames
      }
    ...
    for (meth in cNames)
    {
      ...
      currentLim <- yLim
      plot(x = currentData$start+1,y=currentData[,4],xlim=c(currentIntrv$start,currentIntrv$end),ylim=c(-5,100),pch=20,cex=0.4, cex.main=cex.axis, xlab='', ylab='', yaxt='n', xaxt='n', col=col,main=ifelse(figure_mode,'',setName),xaxs='i',bty="n")
      mtext(setName, side = 4, outer = F,line=0.5,adj=0,las=2,cex=cex.axis*1.2)
