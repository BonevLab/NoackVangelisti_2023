...
submit_averageTAD <- function(main_f,tracks,cells,domains=NULL,domain_size=c(1e5,2e6),file_f,path,bins,stats_f=T,min_dist=1e4,score_tracks){
  cat_string <- paste0('source("',main_f,'config.R")\nsource("',main_f,'scripts/main_functions.R")\nsource("',main_f,'scripts/aux_functions.R")')
  command_f <- paste0('averageTAD(tracks=',deparse(tracks,width.cutoff = 500),',cells=',deparse(cells,width.cutoff = 500),',domains=',deparse(domains,width.cutoff = 500),',domain_size=',deparse(domain_size,width.cutoff = 500),',file_f=',deparse(file_f,width.cutoff = 500),',path=',deparse(path,width.cutoff = 500),',bins=',deparse(bins,width.cutoff = 500),',stats_f=',deparse(stats_f,nlines = 1),',min_dist=',deparse(min_dist,nlines = 1),',score_tracks=',deparse(score_tracks,width.cutoff = 500),')')
  file_r <- paste0('averageTAD_',format(Sys.time(), "%H-%M--%d-%m-%Y"),'.sh')
  cat(paste0(cat_string,'\n',command_f),file=paste0(main_f,'/qsub/',file_r))
  if (sunGrid){
    command_f <- paste0('qsub -l mem=',mem,'G -l h_vmem=',mem,'G -e ',main_f,'logs/averageTAD/',gsub('\\.sh','',file_r),' -o ',main_f,'logs/cis_decay/',paste0(f1,'_',f2,'.',grid_mode,'.',window_i,'.o '),main_f,'/qsub/',paste0(f1,'_',f2,'.',grid_mode,'.',window_i,'_cisDecay.sh'))
  } else {
    command_f <- paste0('nohup Rscript ',main_f,'/qsub/',file_r,' &>',main_f,'logs/averageTAD/',gsub('\\.sh','',file_r),'.log &')
  }
  #print(command_f)
  system(command_f)
}
...
#			center_v <- sum(o[(floor(ncol(o)/2)):(floor(ncol(o)/2)+2),(floor(ncol(o)/2)):(floor(ncol(o)/2)+2)]*grid$total_obs,na.rm=T)/(sum(e[(floor(ncol(e)/2)):(floor(ncol(e)/2)+2),(floor(ncol(e)/2)):(floor(ncol(e)/2)+2)]*grid$total_exp,na.rm=T)/2)
#			left_top <- sum(o[1:3,1:3]*grid$total_obs,na.rm=T)/(sum(e[1:3,1:3]*grid$total_exp,na.rm=T)/2)
#			left_bottom <- sum(o[(nrow(o)-2):(nrow(o)),1:3]*grid$total_obs,na.rm=T)/(sum(e[(nrow(o)-2):(nrow(o)),1:3]*grid$total_exp,na.rm=T)/2)
#			right_top <- sum(o[1:3,(ncol(o)-2):(ncol(o))]*grid$total_obs,na.rm=T)/(sum(e[1:3,(ncol(o)-2):(ncol(o))]*grid$total_exp,na.rm=T)/2)
#			right_bottom <- sum(o[(nrow(o)-2):(nrow(o)),(ncol(o)-2):(ncol(o))]*grid$total_obs,na.rm=T)/(sum(e[(nrow(o)-2):(nrow(o)),(ncol(o)-2):(ncol(o))]*grid$total_exp,na.rm=T)/2)
