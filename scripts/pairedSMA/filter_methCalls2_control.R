...
res <- foreach(chr=chrs, .export=c('chr_sizes','interv1','interv2'),
  ...
  reads_all <- list(read.fst(paste0(path,'/',file_id,'_',chr,'_',1,'.fst')),
                    read.fst(paste0(path,'/',file_id,'_',chr,'_',2,'.fst')))
  for (read_id in 1:2) {
    reads <- reads_all[[read_id]]
  ...
  ### Switch to the other read pair and select matching reads
  for (read_id in c(1,2)) {
    read_id2 <- ifelse(read_id==1,2,1)
    reads <- reads_all[[read_id2]]
    df <- temp_df1[[read_id]]
    
    reg_a <- match(df$read_name,reads$V1)             #Match read IDs
    reg_a <- reg_a[!is.na(reg_a)]
    reg_a <- reg_a[!(reads$V1[reg_a]%in%temp_df1[[read_id2]]$read_name)]            #Make sure reads don't overlap filter interval
    reads <- reads[reg_a,]
    ...
    colnames(df) <- c('read_name','read_id','chr','n_calls','perc_meth',paste0('call_coord',1:ncol(temp_range)),paste0('call',1:ncol(temp_call)),paste0('interv_coord',1:ncol(interv_id)))
    temp_df2[[read_id2]] <- df[order(df$read_name),]   
  }
  
  gc()
  #Now it is time to match read ids and ensure that only pairs overlapping both filter anchors simultaneously are retained
    
    df1 <- merge(temp_df1[[1]],temp_df2[[2]],by='read_name')    #First match read ids for when read1 overlaps left filter anchor and read2 overlaps right filter anchors
    df2 <- merge(temp_df1[[2]],temp_df2[[1]],by='read_name')    #Analogous for read2 overlapping right anchor
